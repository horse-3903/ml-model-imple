...
class LinearRegressionModel:
    def __init__(
            self, 
            w: NDArray[float64] = np.array([]),
            b: float = 0.0,
            X_mean: NDArray[float64] = np.array([]),
            X_std: NDArray[float64] = np.array([]),
            y_mean: float = 0.0,
            y_std: float = 0.0,
        ) -> None:
    ...
    def _cost_function(self, X: NDArray[float64], y: NDArray[float64]):
        m = y.shape[0]
    ...
    def _normalise_y(self, y: NDArray[float64], inplace: bool = False):
        if inplace:
            self.y_mean = y.mean()
            self.y_std = y.std().clip(min=1e-8)
    ...
    def plot_cost(self, ax: Axes):
        ax.plot(self.cost_history, label="Batch Gradient Descent")
        ax.set_xlabel("Epoch")
        ax.set_ylabel("Cost")
        ax.set_title("Cost History Over Epochs")
        ax.legend()

    def plot_predictions_3d(self, ax: Axes3D, X: NDArray[float64], y: NDArray[float64], grid_size: int = 30):
        assert self.has_train
        # scatter actuals
        ax.scatter(X[:, 0], X[:, 1], y, label="Actual", marker='o') # type: ignore
        ...
        # predict on grid (normalization handled inside predict())
        yg = self.predict(Xg)
        yg = self._denormalise_y(yg).reshape(Xg1.shape)

        ax.plot_surface(Xg1, Xg2, yg, alpha=0.4)
        ax.set_title("Predicted plane vs actual points")
...
if __name__ == "__main__":
    ...
    model1.plot_predictions_3d(ax2, X1, y1)

    ax3 = fig.add_subplot(2, 2, 3)
    model2.plot_cost(ax3)

    ax4 = fig.add_subplot(2, 2, 4, projection="3d")
    model2.plot_predictions_3d(ax4, X2, y2)
